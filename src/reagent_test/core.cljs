...
(defn edit-item [e item-id] (swap! app-state assoc-in [item-id :editing] true))

(defn todo-complete [item-id] (swap! app-state assoc-in [item-id :completed] (not (:completed (get @app-state item-id)))))

(defn cancel-edit [e item-id] (swap! app-state assoc-in [item-id :editing] false))

(defn delete-item [item-id] (swap! app-state dissoc item-id))

(defn change-item [event item] (swap! app-state assoc-in [(:id item) :title] (.-value (.-target event))))
